...
# A SECRET_KEY é lida da variável de ambiente (Render) ou usa o fallback (Local)
app.secret_key = os.environ.get('SECRET_KEY', Config.SECRET_KEY_FALLBACK) 

# -------------------- SIMULAÇÃO DE ID DA ONG --------------------
LOGGED_IN_ONG_ID = 1

# -------------------- FUNÇÃO DE CONEXÃO (AJUSTADA PARA RENDER) --------------------
def get_db_connection():
    # Tenta obter a URL de conexão do ambiente (usada pelo Render/Produção)
    DATABASE_URL = os.environ.get('DATABASE_URL')
    
    if DATABASE_URL:
        # Conecta usando a URL de conexão completa (padrão URI)
        return psycopg2.connect(DATABASE_URL)
    else:
        # Conecta usando as configurações de desenvolvimento local (do config.py)
        # Note: Esta é a conexão de DEV, no Render a URL será usada.
...
def login():
    if request.method == 'POST':
        ...
        # Comparação direta da senha (sem hash) - ATENÇÃO: RECOMENDADO USAR check_password_hash
...
def cadastrar_curso():
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        # conn.close() # Removido, pois a conexão é feita dentro do bloco 'with' no get_db_connection
        # cur.close() # Removido
...
def acesso_funcionarios():
    if request.method == 'POST':
        ...
        # ATENÇÃO: É essencial usar generate_password_hash aqui. 
        # Seu código original já o importava.
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                """, (usuario, senha_hash, cargo, nivel, ativo))
                conn.commit()
...
# -------------------- RODAR APP --------------------
# O bloco if __name__ == '__main__' é removido para que o Gunicorn possa carregar a aplicação.
